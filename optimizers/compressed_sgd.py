    ...
    for group in self.param_groups:
      for p in group['params']:
        ...
        if self.binning == 'lin':
          ...
          grad[grad > 0] = torch.ceil((2**(self.num_bits-1)) * (torch.div(torch.log(grad[grad > 0] + 1.0001), torch.log(self.max_grad_vals[p][grad > 0] + 1.0001)))) # log_e(x+1)/log_e(maxgrad+1) = log_(maxgrad+1)(x+1)
          grad[grad < 0] = -torch.ceil((2**(self.num_bits-1)) * (torch.div(torch.log(-grad[grad < 0] + 1.0001), torch.log(-self.min_grad_vals[p][grad < 0] + 1.0001))))
